...
Options:
  ...
  --notes-namespace=<namespace>     Metadata generated by fusionner is stored as Git notes.
                                    Namespace for the Git notes that fusionner will create. Defaults to `fusionner`
...
struct Args {
    ...
    flag_notes_namespace: Option<String>,
...
fn main() {
    let return_code;
    {
        ...
        return_code = match process(&config,
                                    &watch_refs,
                                    &args.flag_target_reference,
                                    &args.flag_remote,
                                    &args.flag_notes_namespace) {
            Ok(_) => 0,
            Err(err) => {
                error!("Error: {}", err);
                1
